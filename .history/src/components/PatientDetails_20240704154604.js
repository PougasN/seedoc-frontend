      ...
      if (data.resourceType === "DiagnosticReport") {
        ...
        const imagePromises = data.presentedForm.map((form, index) => {
          return new Promise((resolve) => {
            ...
            img.src = form.url;
            img.onload = () => {
              const imgWidth = img.width > 180 ? 180 : img.width; // Fit width to 180 if wider
              const imgHeight = (img.height / img.width) * imgWidth; // Maintain aspect ratio
              doc.addImage(img, 'PNG', 10, 70 + (index * (imgHeight + 10)), imgWidth, imgHeight); // Adjust the positioning as needed
              resolve();
            };
          });
        });
