...
const PatientDetails = () => {
      ...
      try {
        ...
        if (encounterData.entry) {
          const encounterList = await Promise.all(encounterData.entry.map(async entry => {
            ...
            const diagnosticReport = diagnosticReportResponse.ok ? await diagnosticReportResponse.json() : null;
            ...
            return {
              ...
              status,
              date,
              videoUploaded: resource.extension ? resource.extension.some(ext => ext.url === 'http://example.com/fhir/StructureDefinition/videoUploaded' && ext.valueBoolean) : false,
              readEnabled: diagnosticReport != null,
  ...
  const handleReadClick = (encounterId) => {
    ...
    const mediaIdExtension = encounter.extension.find(ext => ext.url === 'http://example.com/fhir/StructureDefinition/mediaId');
    if (!mediaIdExtension) {
      console.error('Encounter has no mediaId extension', encounter);
      return;
    }

    const mediaId = mediaIdExtension.valueReference.reference.split('/')[1];
    navigate(`/video/${encounterId}/${mediaId}`);
  };
  ...
  const generatePDF = async (encounter) => {
    ...
    if (encounter.dReport) {
      doc.text(`Conclusion: ${encounter.dReport.conclusion}`, 10, 60);

      const attachments = encounter.dReport.presentedForm;
      let yPosition = 70;
              ...
              <tr key={index} className="encounter-row">
                ...
                <td>
                  {encounter.readEnabled ? (
                    <button onClick={() => handleReadClick(encounter.id)}>Read</button>
                  ) : (
