...
const generatePdfDocument = (patient, encounter) => (
  <Document>
    <Page>
      <Text>Patient Name: {patient}</Text>
      <Text>Encounter Description: {encounter.description}</Text>
      <Text>Encounter Status: {encounter.status}</Text>
      <Text>Encounter Date: {new Date(encounter.date).toLocaleString()}</Text>
    </Page>
  </Document>
);
...
const PatientDetails = () => {
  ...
  const handleStatusSave = () => {
    ...
    const updatedEncounter = { ...selectedEncounter, status: newStatus };

    fetch(`http://localhost:9090/encounter/${selectedEncounter.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/fhir+json',
      },
      ...
      .then(response => response.json())
      .then(() => {
        setEncounters(encounters.map((enc, i) => i === selectedEncounterIndex ? updatedEncounter : enc));
          ...
          {encounters.length === 0 ? (
            ...
            encounters.map((encounter, index) => (
              <tr key={index}>
                ...
                <td>
                  <PDFDownloadLink
                    document={generatePdfDocument(patient, encounter)}
                    fileName={`encounter_${encounter.id}.pdf`}
                  >
                    {({ loading }) =>
                      loading ? 'Loading document...' : <img src={pdfIcon} alt="PDF icon" />
                    }
                  </PDFDownloadLink>
                </td>
              </tr>
