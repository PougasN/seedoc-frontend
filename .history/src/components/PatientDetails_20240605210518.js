...
const PatientDetails = () => {
  ...
  const handleReadClick = (videoUrl, encounter) => {
    navigate('/video', {
      state: {
        videoUrl: encodeURIComponent(videoUrl),
        patientName: patient,
        birthDate: '01/01/1970',  // Replace with actual birth date if available
  ...
  const generatePDF = (encounter) => {
    const doc = new jsPDF();
    doc.text(`Patient Name: ${patient.name}`, 10, 10);
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      {encounter.readEnabled ? (
                        <button onClick={() => handleReadClick(encounter.videoUrl, encounter)}>Read</button>
                      ) : (
