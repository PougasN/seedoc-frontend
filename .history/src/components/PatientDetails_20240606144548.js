...
const PatientDetails = () => {
  ...
  const generatePDF = (encounter) => {
    ...
    doc.text(`Encounter ID: ${encounter.id}`, 10, 20);
    doc.text(`Description: ${encounter.description}`, 10, 30);
    doc.text(`Status: ${encounter.status}`, 10, 40);
    doc.text(`Date: ${new Date(encounter.date).toLocaleString('en-GB', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
      hour: '2-digit',
    ...
    <div className="fixed-width">
      ...
      <table>
        <thead>
          <tr>
            <th>#</th>
            <th onClick={() => handleSort('id')}>Encounter ID</th>
            <th>Description</th>
            <th>Status</th>
          ...
          {sortedEncounters.length === 0 ? (
            ...
            sortedEncounters.map((encounter, index) => (
              <tr key={index}>
                ...
                <td>{new Date(encounter.date).toLocaleString('en-GB', {
                  day: '2-digit',
                  month: '2-digit',
                  year: 'numeric',
                  hour: '2-digit',
