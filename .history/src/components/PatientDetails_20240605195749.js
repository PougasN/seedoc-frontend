...
import pdfIcon from '../assets/pdf-icon.png'; // Ensure this path is correct and the file exists
...
const PatientDetails = () => {
  ...
  const handleStatusSave = () => {
    ...
    updatedEncounters[selectedStatusEncounter].status = newStatus;

    fetch(`http://localhost:9090/encounter/${updatedEncounters[selectedStatusEncounter].id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/fhir+json',
      },
      body: JSON.stringify({
        ...updatedEncounters[selectedStatusEncounter],
        status: newStatus,
      }),
    })
      .then(response => response.json())
