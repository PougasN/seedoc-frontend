...
const PatientDetails = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:9090/patient/${patientId}/encounters`)
      ...
      .catch(error => {
        ...
        setEncounters([]); // Handle error state
      });
    ...
    fetch(`http://localhost:9090/media`)
      ...
      .then(mediaData => {
        if (mediaData.entry) {
          const mediaReferences = mediaData.entry.map(entry => entry.resource.encounter.reference.split('/')[1]);

          setEncounters(prevEncounters =>
            prevEncounters.map(encounter => {
              const videoUploaded = mediaReferences.includes(encounter.id);
              return { ...encounter, videoUploaded, readEnabled: videoUploaded };
            })
          );
        }
      })
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      console.log('data after uploading video = ', data.url);
  ...
  const handleReadClick = (videoUrl) => {
    console.log(`Video URL while going to videoplayer: `, encodeURIComponent(videoUrl)); // Log the mediaId here
