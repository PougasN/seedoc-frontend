...
const VideoPlayer = () => {
  ...
  useEffect(() => {
    const fetchMedia = async () => {
      try {
        console.log(`Media ID:`, videoUrl); // Log the mediaId here
        const response = await fetch(`http://localhost:9090/media?videoUrl=${videoUrl}`);
        const data = await response.json();
        if (data.length > 0) {
          setMediaId(data[0].id);
          console.log(`Fetched media with ID: ${data[0].id}`); // Log the mediaId here
