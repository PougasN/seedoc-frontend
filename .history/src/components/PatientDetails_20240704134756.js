...
const PatientDetails = () => {
  ...
  const handleReadClick = (encounter) => {
    if (!encounter.videoUrl) {
      console.error('Encounter has no videoUrl', encounter);
      return;
    }
  
    const mediaExtension = encounter.extension.find(ext => ext.url === 'http://example.com/fhir/StructureDefinition/mediaId');
    if (!mediaExtension) {
      console.error('No mediaId extension found in encounter', encounter);
      return;
    }
