...
const VideoPlayer = () => {
  ...
  const handleFrameForward = () => {
    playerRef.current.seekTo(playerRef.current.getCurrentTime() + 1 / 5, 'seconds'); // Assuming 30fps, move forward by one frame
  };

  const handleFrameBack = () => {
    playerRef.current.seekTo(playerRef.current.getCurrentTime() - 1 / 5, 'seconds'); // Assuming 30fps, move backward by one frame
  };
