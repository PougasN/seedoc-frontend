...
const VideoPlayer = () => {
  ...
  useEffect(() => {
    const fetchMedia = async () => {
      try {
        const response = await fetch(`http://localhost:9090/media?videoUrl=${encodeURIComponent(videoUrl)}`);
        const data = await response.json();
        if (data) {
          setMediaId(data.id);
          fetchFindings(data.id);
        }
      } catch (err) {
  ...
  const handleSaveComment = async () => {
    ...
    formData.append('frame', dataURLtoFile(currentFrame, 'frame.png'));
